# Instance Management Spec

## Purpose

Defines how ControlAI manages `ControlaiInstance` rows — both BYO (Bring-Your-Own daemon, registered via `instance.register`) and managed (auto-provisioned via `instance.provision`) — including the pluggable `InstanceProvisioner` backend, retry/deprovision flows, URL derivation, and auto-cleanup of failed provisions.
